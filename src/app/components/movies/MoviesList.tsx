...
import { moviesGroupByDate, moviesGroupByProvider } from '@/utils/movies-utils'
...
export const MoviesList = ({ movies }: Props) => {
  const data = moviesGroupByDate(movies)
    ...
    return (
      <div key={item.id} className="mb-6">
        <div className="flex flex-row justify-between items-center">
          <div className="py-8 flex flex-row items-center space-x-3 text-2xl">
            {/* <Image src={`/providers/${streamProvider?.icon}`} width="28" height="28" className="rounded-md" alt={streamProvider?.name ?? ''} /> */}
            <span>{date}</span>
          </div>
        </div>

        <div id={date} className="space-x-2 lg:space-x-4 overflow-x-auto whitespace-nowrap w-full scroll-smooth flex flex-row">
